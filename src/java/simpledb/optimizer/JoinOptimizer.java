...
public class JoinOptimizer {
     ...
     * much about what the size of the output is -- it could be the size
     * of the product of the cardinalities of the tables (if both tables
     * have the same value for all tuples) -- or it could be 0. It's fine
     * to make up a simple heuristic (say, the size of the larger of the two tables).
     * <p>
     * For range scans, it is similarly hard to say anything accurate about
     * sizes. The size of the output should be proportional to the sizes of
     * the inputs. It is fine to assume that a fixed fraction of the cross-product
     * is emitted by range scans (say, 30%). In general, the cost of a range join
     * should be larger than the cost of a non-primary key equality join of two tables of the same size.
     */
    ...
    public List<LogicalJoinNode> orderJoins(
        ...
        for (int i = 1; i <= size; i++) {
            for (Set<LogicalJoinNode> s : enumerateSubsets(joins, i)) { //得到固定长度i的子集
                //遍历集合中的集合，得到集合中的每个集合的最小
                double bestCost = Double.MAX_VALUE;//长度i的最低cost
                bestCostCard = new CostCard();
                for (LogicalJoinNode logicalJoinNode : s) {
                    //计算 logicalJoinNode 与 其他node的join 结果
